...
namespace PhysIKA
{
	...
	__device__ Real HPM_Hardening(Real rho)
	{
		Real hardening = 1.0f;
	...
	template <typename Real, typename Coord, typename Matrix, typename NPair>
	__global__ void HPM_ReconstructRestShape(
	...
	__global__ void HPM_ReconfigureRestShape(
	...
	template <typename Real, typename Coord, typename Matrix, typename NPair>
	__global__ void HPM_ComputeInverseDeformation(
		DeviceArray<Matrix> invF,
		DeviceArray<Coord> position,
		NeighborList<NPair> restShape,
		Real horizon)
	...
	__global__ void HPM_EnableAllReconstruction(
		DeviceArray<bool> bYield)
	{
		int i = threadIdx.x + (blockIdx.x * blockDim.x);
	...
	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::reconstructRestShape()
	{
		...
		{
			HPM_EnableAllReconstruction << <pDims, BLOCK_SIZE >> > (m_bYield);
		}
		...
		HPM_ReconfigureRestShape << <pDims, BLOCK_SIZE >> > (
			index,
			m_bYield,
		...
		HPM_ComputeInverseDeformation << <pDims, BLOCK_SIZE >> > (
			m_invF,
			this->inPosition()->getValue(),
			this->m_restShape.getValue(),
			this->inHorizon()->getValue());

		HPM_ReconstructRestShape<< <pDims, BLOCK_SIZE >> > (
			newNeighborList,
			m_bYield,
			this->inPosition()->getValue(),
			m_I1,
	...
	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::enableFullyReconstruction()
	{
	...
	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::disableFullyReconstruction()
	{
		m_reconstuct_all_neighborhood.setValue(false);
	}
	...
	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::enableIncompressibility()
	{
		m_incompressible.setValue(true);
	}

	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::disableIncompressibility()
	{
	...
	template <typename Real, typename Coord, typename Matrix, typename NPair>
	__global__ void HPM_RotateRestShape(
		DeviceArray<Coord> position,
		DeviceArray<bool> bYield,
	...
	template<typename TDataType>
	void HyperelastoplasticityModule<TDataType>::rotateRestShape()
	{
		int num = this->inPosition()->getElementCount();
		uint pDims = cudaGridSize(num, BLOCK_SIZE);

		HPM_RotateRestShape <Real, Coord, Matrix, NPair> << <pDims, BLOCK_SIZE >> > (
			this->inPosition()->getValue(),
			m_bYield,
			this->m_restShape.getValue(),
			this->inHorizon()->getValue());
